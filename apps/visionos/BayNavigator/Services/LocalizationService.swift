...
enum AppLocale: String, CaseIterable, Identifiable {
    ...
    static func from(code: String) -> AppLocale {
...
/// Localization service for Bay Navigator visionOS app
/// Fetches translations from the web API and caches them locally
actor LocalizationService {
    ...
    // Current locale
    private(set) var currentLocale: AppLocale = .en
    ...
    nonisolated func getCurrentLocale() -> AppLocale {
    ...
    func loadTranslations(for locale: AppLocale) async {
    ...
    func t(_ key: String, params: [String: Any]? = nil) -> String {
    ...
    func preloadAllTranslations() async {
        for locale in AppLocale.allCases {
            await loadTranslations(for: locale)
        }
    }
